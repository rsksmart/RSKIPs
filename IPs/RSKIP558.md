...
Users, such as rollup operators, first encode data into blobs. Each blob represents a set of `FIELD_ELEMENTS_PER_BLOB`(i.e. 4096) *field elements*, each of which is of size `BYTES_PER_FIELD_ELEMENT` (i.e. 32 bytes). Each blob must therefore be exactly 128KB, otherwise the existent cryptographic tooling will not work. This is why we cannot start with  smaller blobs on Rootstock - as that would require its own cryptographic parameters, trusted setup ceremony and sofware library.
...
The field `max_fee_per_blob_gas` is a `uint256` and the field `blob_versioned_hashes` represents a list of hash outputs from `kzg_to_versioned_hash` , one per each blob attached (see note about helper function above). There must be at least one blob attached - this restriction is not included in EIP-4844, but it is enforced in [Geth](https://github.com/ethereum/go-ethereum/blob/89c1c16a46ce6390d53f8d17cd56caca9c2b15ac/core/txpool/validation.go#L182).
...
\[4] [RSKIP-535: Block header extension mechanism](https://github.com/rsksmart/RSKIPs/blob/master/IPs/RSKIP535.md)
