...
7. A transaction with no authorization list calls a precompiled contract with a gas limit below the cost the contract declares for the input. Before and after activation the receipt status is `0`, the gas used and the fee are the gas limit, and no state change or log is recorded.
8. Where RSKIP545 is active, a set-code transaction with one valid authorization calls a precompiled contract that fails. After activation the authorization is processed, the receipt status is `0`, and the gas used and the fee equal the gas limit minus the refund that RSKIP545 grants for the authorization.
9. Where RSKIP545 is active, a set-code transaction with one valid authorization calls a precompiled contract with a gas limit that covers the intrinsic cost and the authorization but not the cost the contract declares for the input. Before activation the receipt status is `0` and the gas used and the fee are the gas limit. After activation the receipt status is `0`, and the gas used and the fee equal the gas limit minus the refund that RSKIP545 grants for the authorization.
