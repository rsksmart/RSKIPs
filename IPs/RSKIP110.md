...
The CPV field is a vector V with elements v(i) (i = 0..6) where each v(i) consist of the the least significant byte (LSB) of the Bitcoin Block ID associated with the RSK block at height ( floor(BN/64)\*64 - i\*64 ).

The number of uncles (NU) can be zero if the last 32 blocks do not reference any uncle. The uncles referenced can be siblings of blocks that are prior to the last 32 blocks. The block being mines also counts. Each block can reference a maximum of 7 uncles, so 32 blocks can only reference 224 uncles, so 1 bytes is enough.
